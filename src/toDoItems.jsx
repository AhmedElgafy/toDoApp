...
const ToDoItems=({styling,setStyling,data,numOfItems
                  ,setToDoList,setItemsLeft})=>{

  //------------------------states-------------------
    const [filter,setFilter]=useState("")
    const [status,setStatus]=useState([ {id:Math.random(),data:"All",nameClass:"active"},
  ...
  useEffect(()=>{                                            
    setViewList(
      ...
      ,[data])
      const [viewList,setViewList]=useState( data.map(ele=><ToDoItem ele={ele} key={ele.id}
        setItemsLeft={setItemsLeft}
        data={data}
        styling={styling}
        setStyling={setStyling}
    ...
    return(
        ...
        <div className={ ' toDoContainer '+styling.inputStyle}>
          ...
          <div className={"toDoCreator status sm-font "+styling.inputStyle}>
            ...
            <div id="statusMode" className="inside item-center">
              {status.map(ele=><h4 key={ele.id} onClick={e=>activeToggle(e)} id={ele.id}
                                    className={ele.nameClass}>{ele.data}</h4>)}
            </div>
            <p onClick={clearzList} className='item-right'>Clear Completed</p>
          </div>
        </div>
        </>
